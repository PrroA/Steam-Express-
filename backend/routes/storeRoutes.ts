...
export function registerStoreRoutes({ app, state, authenticate, isAdmin }: RouteDeps) {
  ...
  const isValidImageSignature = (buffer: Buffer, contentType: string) => {
  ...
  app.post(
    ...
    (req: TypedAuthRequest, res: Response) => {
      ...
      if (!isValidImageSignature(req.body, contentType)) {
        return res.status(400).json({ message: '圖片內容與檔案格式不符' });
      }
