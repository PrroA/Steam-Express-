  ...
  return (
    <section className="steam-panel mt-5 rounded-2xl p-5 md:p-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-bold tracking-[0.14em] text-[#8fb8d5]">營運提醒</p>
          <h2 className="mt-1 text-xl font-black text-[#d8e6f3]">需要優先處理的項目</h2>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        ...
        <StatusTile label="低庫存項目" value={lowStockItems.length} tone="danger" />
        <StatusTile label="未上架商品" value={inactiveGames.length} tone="muted" />
      </div>
              ...
              paidWaitingShipmentOrders.slice(0, 3).map((order) => (
                <div key={order.id} className="rounded-md border border-[#66c0f433] bg-[#102131] p-3">
                  ...
                  <p className="mt-1 text-xs text-[#9eb4c8]">
                    使用者 #{order.userId} · ${order.total.toFixed(2)} · {formatOrderTime(order.date)}
                  </p>
                  <p className="mt-1 text-xs font-semibold text-[#8fd1ff]">
                    建議：確認付款後安排出貨，並更新出貨狀態。
                  </p>
                </div>
              ))
            ) : (
              ...
              lowStockItems.slice(0, 4).map((item) => (
                <div key={`${item.gameId}-${item.variantName}`} className="rounded-md border border-[#ffcf5a55] bg-[#2c2617] p-3">
                  ...
                  <p className="mt-1 text-xs text-[#f0c674]">
                    {item.variantName} 剩下 {item.stock} 件
                  </p>
                </div>
              ))
            ) : (
