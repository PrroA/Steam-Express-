...
我這次整理的重點不是新增大量功能，而是讓整個流程可以穩定展示，並把 AI 做成可解釋的客服、商品推薦與購買決策功能。正式環境不放 OpenAI key 時，商品頁也有 Browser-side AI POC 可以展示零 API 成本的 AI 互動入口。
...
> 另外，商品頁與比較頁的本機決策助理是為了處理展示成本問題。OpenAI 品質好但會花錢，Ollama 本機可用但部署後不會自動存在；所以我加了一層瀏覽器端 AI，能用就用，不能用就回到商品資料分析，而且畫面會清楚顯示目前使用哪一種分析方式。
