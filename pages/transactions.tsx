...
import type { Transaction } from '../services/orderService';
    ...
    return (
      <main className="steam-shell flex min-h-screen items-center justify-center px-4 py-10">
        <div className="steam-panel w-full max-w-xl rounded-2xl p-10 text-center">
          ...
          <p className="mt-4 text-sm text-[#9eb4c8]">正在載入交易紀錄...</p>
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="steam-shell flex min-h-screen items-center justify-center px-4 py-10">
        <div className="steam-panel w-full max-w-xl rounded-2xl p-10 text-center">
          ...
          <h1 className="mt-4 text-3xl font-black text-[#d8e6f3]">目前還沒有交易紀錄</h1>
          <p className="mt-2 text-[#9eb4c8]">完成付款後，會在這裡看到每筆交易明細。</p>
  ...
  return (
    <main className="steam-shell px-4 py-6 md:px-6">
      <section className="mx-auto w-full max-w-5xl">
        <h1 className="text-3xl font-black text-[#d8e6f3]">交易紀錄</h1>
            ...
            {transactions.map((txn) => (
              ...
              >
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm font-bold text-[#d8e6f3]">交易編號：{txn.transactionId}</p>
                    <p className="text-xs text-[#9eb4c8]">
                      付款時間：{new Date(txn.paidAt).toLocaleString()}
                    </p>
