...
async function queryOllama(messages, temperature = 0.7) {
    try {
...
async function queryAssistant({ openaiClient, messages, temperature = 0.7, openaiModel = 'gpt-4o-mini', }) {
    if (openaiClient) {
        try {
...
function isPlatformQuery(message) {
    return /(訂單|退款|退費|取消|購物車|付款|支付|結帳|物流|運送|出貨|登入|帳號|密碼|願望清單|收藏|商品|遊戲|版本|庫存|價格|管理員|後台|平台|客服|交易|狀態|stripe|payment|order|refund|cart|wishlist|checkout|login|account|admin)/i.test(message);
}
        ...
        try {
            const result = await queryAssistant({
                ...
                messages: [
                    {
                        role: 'system',
                        content: '你是 Steam 風格電商平台客服助理。請用繁體中文，回覆精簡、可執行，優先提供實際操作步驟。',
                    },
            ...
            try {
                const result = await queryAssistant({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: '你是一位友善、自然的繁體中文 AI 助手。可以一般聊天，也可回答技術與生活問題。回覆精簡、自然，不要制式客服語氣。',
                        },
                        { role: 'user', content: message },
                    ],
                });
                if (!result) {
                    return res.json({
                        reply: '目前一般聊天服務暫時不可用。若你想，我可以先提供平台相關協助（訂單、退款、付款、物流）。',
                        grounded: false,
            ...
            try {
                const result = await queryAssistant({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: '你是平台客服助理。當內部知識不足時，請誠實說明資料不足，並提供下一步檢查建議；不可編造政策。',
                        },
                        { role: 'user', content: message },
                    ],
                });
                if (!result) {
                    return res.json({
                        reply: '目前找不到對應平台資料。你可以提供更多細節（例如訂單狀態、付款步驟、商品名稱），我再幫你判斷。',
                        grounded: false,
