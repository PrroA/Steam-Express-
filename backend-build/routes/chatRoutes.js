...
const serviceScopePattern = /(商品|遊戲|推薦|價格|庫存|版本|購物車|結帳|付款|訂單|退款|取消|出貨|配送|物流|帳號|登入|註冊|密碼|願望清單|客服|game|price|cart|checkout|payment|order|refund|shipping|wishlist|account|login)/i;
...
function buildFallbackReply(message, grounded, contexts = []) {
...
function buildSystemPrompt(grounded) {
    const base = '你是遊戲商城的 AI 客服助理。請用繁體中文回答，語氣自然、簡潔、像真正客服。不要提到後端、API、server、token、模型錯誤或工程細節。不要執行付款、退款、取消訂單，只能提供導引。';
    if (!grounded) {
