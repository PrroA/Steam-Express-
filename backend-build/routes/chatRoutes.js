...
const serviceScopePattern = /(商城|商品|推薦|便宜|低價|預算|遊戲|價格|庫存|版本|購物車|結帳|付款|訂單|退款|配送|出貨|帳號|登入|願望清單|客服|game|price|cart|checkout|payment|order|refund|shipping|wishlist|account|login)/i;
const recommendationPattern = /(推薦|便宜|低價|預算|好玩|新手|熱門|遊戲|商品|庫存|版本|recommend|cheap|price|game|stock)/i;
const productSearchPattern = /(找|搜尋|查詢|篩選|有沒有|想玩|適合|新手|放鬆|劇情|多人|合作|恐怖|不要|以下|預算|便宜|search|find|filter|looking for)/i;
const productDecisionPattern = /(比較|比一比|哪一款|哪款|怎麼選|該買|適合|預算|不超過|以內|推薦.*還是|compare|versus|\bvs\b|which|choose|budget|under|below|for me)/i;
...
const orderCarePattern = /(訂單.*接下來|訂單.*怎麼辦|訂單.*退款|這筆.*退款|這筆.*怎麼辦|付款後.*怎麼辦|付款後.*處理|售後|after.*order|order.*next|refund.*order)/i;
const orderLookupPattern = /(我的訂單|訂單狀態|查訂單|最近訂單|訂單進度|付款狀態|出貨狀態|my order|order status)/i;
async function queryOllama(messages, temperature = 0.4) {
    try {
...
async function queryAssistant({ openaiClient, messages, temperature = 0.4, }) {
    if (openaiClient) {
        try {
...
function getOptionalUser(req, secretKey) {
...
function isProductRecommendationQuestion(message) {
    return recommendationPattern.test(message);
}
function isProductSearchQuestion(message) {
    if (!productSearchPattern.test(message))
        return false;
    if (/推薦|recommend/i.test(message))
        return false;
    if (!/(找|搜尋|查詢|篩選|有沒有|想玩|想找|search|find|filter|looking for)/i.test(message))
        return false;
...
function isProductDecisionQuestion(message) {
    const hasDecisionQualifier = /(比較|比一比|哪一款|哪款|怎麼選|該買|預算|不超過|低於|小於|以內|compare|versus|\bvs\b|which|choose|budget|under|below)/i.test(message);
    return hasDecisionQualifier && productDecisionPattern.test(message) && recommendationPattern.test(message);
}
function isProductComparisonQuestion(message, state) {
    return productComparisonPattern.test(message) && findMentionedGames(state.games, message).length >= 2;
}
function isCartReviewQuestion(message) {
    return cartReviewPattern.test(message);
}
function isOrderCareQuestion(message) {
...
function getCartItemUnitPrice(item) {
    if (item.variantId && item.variants) {
        ...
        if (selectedVariant)
            return toPrice(selectedVariant.price);
    }
    return getGameLowestPrice(item);
}
function getGameLowestPrice(game) {
...
function isGameMentioned(game, message) {
...
function getPersonalizedScore(game, profile) {
    if (!profile?.hasSignals)
        return 0;
    let score = 0;
    if (profile.wishlistIds.has(game.id))
        score += 24;
...
function getPersonalizedReasons(game, profile) {
    ...
    if (!profile?.hasSignals)
        return reasons;
    if (profile.wishlistIds.has(game.id))
        reasons.push('在你的願望清單裡');
    if (profile.cartIds.has(game.id))
        reasons.push('你已經放進購物車');
    const overlap = getGameTerms(game).filter((term) => profile.interestTerms.has(term)).length;
...
function extractDecisionTerms(message) {
...
function extractProductSearchIntent(message) {
    const lowerMessage = message.toLowerCase();
    const labels = [];
    const terms = new Set();
    const avoidTerms = new Set();
    const addGroup = (label, groupTerms, patterns) => {
    ...
    const budgetMatch = normalized.match(/(?:預算|低於|不超過|以下|under|below|max|budget)\s*\$?\s*(\d+(?:\.\d+)?)/i) ||
...
function scoreGameForSearch(game, intent, profile) {
    const price = getGameLowestPrice(game);
    ...
    if (stock > 0)
        score += 8;
    if (intent.onlyInStock && stock <= 0)
        score -= 80;
    if (intent.budgetMax !== null) {
        if (price <= intent.budgetMax)
            score += 30 + Math.max(0, intent.budgetMax - price) / 4;
        else
            score -= Math.min((price - intent.budgetMax) * 2, 60);
    }
    ...
    if (intent.labels.length === 0 && /rpg|fantasy|adventure/i.test(gameText))
        score += 3;
    return score;
}
function buildSearchReason(game, intent, profile) {
    const reasons = [];
    const price = getGameLowestPrice(game);
    const stock = getAvailableStock(game);
    const gameText = `${game.name} ${game.description || ''}`.toLowerCase();
    ...
    const matchedLabels = intent.labels.filter((label) => {
        if (label.includes('RPG'))
            return /rpg|fantasy|magical/i.test(gameText);
        if (label.includes('開放'))
            return /open|world|adventure/i.test(gameText);
        if (label.includes('劇情'))
            return /legendary|experience|survival/i.test(gameText);
        if (label.includes('輕鬆'))
            return /magical|experience|adventure/i.test(gameText);
        if (label.includes('恐怖'))
            return /horror|survival/i.test(gameText);
        return false;
    });
    if (matchedLabels.length > 0)
        reasons.push(`符合「${matchedLabels.slice(0, 2).join('、')}」`);
    if (stock > 0)
...
function buildDecisionReasons({ game, budget, decisionTerms, profile, }) {
    const price = getGameLowestPrice(game);
    const stock = getAvailableStock(game);
    const gameText = `${game.name} ${game.description || ''}`.toLowerCase();
    const reasons = [];
...
function scoreGameForDecision({ game, budget, decisionTerms, profile, }) {
    const price = getGameLowestPrice(game);
    const stock = getAvailableStock(game);
    const gameText = `${game.name} ${game.description || ''}`.toLowerCase();
    let score = getPersonalizedScore(game, profile);
    if (stock > 0)
        score += Math.min(stock / 4, 10);
    if (budget !== null) {
        if (price <= budget) {
            score += 35;
...
function buildProductDecision(state, message, user, clientProfile) {
    ...
    const sources = picks
        .slice(0, 2)
        .map((game, index) => createCatalogSource(game, index, index === 0 ? '最符合這次的預算與偏好' : '可當作替代選項比較'));
    if (picks.length === 0) {
        return {
...
function describeFit(game, message, profile) {
    const gameText = `${game.name} ${game.description || ''}`.toLowerCase();
    const hints = [];
    ...
    if (/rpg|fantasy|adventure/i.test(gameText))
        return '適合想玩角色扮演或冒險的人';
    return '適合想先從價格與庫存穩定的商品開始';
}
function describeTradeoff(game, message) {
    const price = getGameLowestPrice(game);
    const stock = getAvailableStock(game);
    const budget = extractBudget(message);
    if (budget !== null && price > budget)
        return `最低價格超出 $${budget.toFixed(2)} 預算`;
    if (stock <= 0)
        return '目前沒有可購買庫存';
    if (stock < 10)
        return '庫存較少，想買要盡快決定';
...
function buildProductComparison(state, message, user, clientProfile) {
    ...
    const comparison = ranked.map((game) => ({
        gameId: game.id,
        name: game.name,
        price: money(getGameLowestPrice(game)),
    ...
    const lines = comparison.map((row) => `${row.name}：${row.price}，庫存 ${row.stock}，適合點：${row.fit}，取捨：${row.tradeoff}`);
    const reply = [
        `我會先選 ${topPick.name}。`,
...
function buildRecommendation(state, message, user, clientProfile) {
    ...
    const ranked = [...activeGames].sort((a, b) => {
        ...
        if (cheapIntent)
            return getGameLowestPrice(a) - getGameLowestPrice(b);
    ...
    const intro = profile?.hasSignals
...
async function polishRecommendationReply({ openaiClient, message, recommendation, }) {
    const result = await queryAssistant({
        ...
        messages: [
            {
                role: 'system',
                content: '你是遊戲商城的 AI 選品顧問。請用自然繁體中文回覆，像真人客服在推薦商品。只能根據提供的候選商品與原因回答，不要捏造評分、折扣或不存在的特色。不要出現 API、server、token、模型、provider 等工程字眼。回覆 3 到 6 句，可用條列但不要太機械。',
            },
            {
                role: 'user',
...
function buildOrderStatusReply(orders) {
    if (orders.length === 0) {
        return {
    ...
    const lines = latest.map((order, index) => {
        ...
        const actionHint = order.status === 'pending'
            ? '可以回到訂單詳情繼續付款。'
...
function getCartItemAdvice(item, lineTotal, total) {
...
function buildCartReview(cart) {
    ...
    const nextStep = total >= 90 && largest
        ? `如果想壓低預算，可以先確認 ${largest.item.name} 是否現在就要買。`
        : '如果內容都確認了，可以前往結帳建立訂單。';
    const items = sortedRows.slice(0, 3).map(({ item, quantity, lineTotal }) => ({
        gameId: item.id,
        name: item.name,
        quantity,
    ...
    const itemLines = items.map((item) => `${item.name}${item.variantName ? `（${item.variantName}）` : ''} x${item.quantity}：${item.lineTotal}，${item.advice}`);
    return {
...
function getOrderCareAction(order) {
...
function buildOrderCare(orders) {
...
function buildFallbackReply(message, grounded) {
...
function buildSystemPrompt(grounded) {
    const base = '你是 Steam Practice 商城的客服助理。請用自然繁體中文回答一般使用者，不要出現 API、server、token、PaymentIntent、500 等工程字眼。不要替使用者執行付款、退款、取消訂單或修改資料，只能說明下一步。';
    if (!grounded) {
        return `${base} 如果問題不在商城客服範圍，請簡短引導回商品、購物車、付款、訂單、退款、配送、帳號或願望清單。`;
    }
    return `${base} 請只根據提供的商城資料回答；如果資料不足，請說明可以到商店頁或訂單中心確認。回答控制在 4 到 8 句。`;
}
function shouldIncludeRagDebug(req) {
...
function buildRagDebug(query, matches = []) {
    return {
...
function registerChatRoutes({ app, io, state, openaiClient, secretKey }) {
